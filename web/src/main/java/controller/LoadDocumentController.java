...
public class LoadDocumentController extends HttpServlet {
	...
	// 	END  CHANGE MTURK
	    ...
	    session.setAttribute(Controller.ORIG_TEXT, text); 
		System.out.println("Text Load = " + text + "HitId=" + hitId);
		if (text == null) {
			text = "Error!!! Please reload page or return the HIT and contact the requester";
		}
		END  CHANGE MTURK
		 */
	...
	private void doRedo(HttpServletResponse response, String hitIdWorkerId, HttpSession session) throws IOException {
	...
	private void doUndo(HttpServletResponse response, String hitIdWorkerId, HttpSession session) throws IOException {
